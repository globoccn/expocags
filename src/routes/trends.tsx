...
function periodTickLabel(period: PeriodKey, index: number) {
  ...
  if (period === "week")
    return [`13/06`, `14/06`, `15/06`, `16/06`, `17/06`, `18/06`, `19/06`][index] || `D-${index}`;
...
function getSelectedChillers(group: GroupKey, source: ChillerData[]) {
  if (group === "all") return source;
  return source.filter((c) => c.id === group);
}
...
const trendLines: Record<
  ContextKey,
  Array<{ key: string; label: string; color: string; dashed?: boolean }>
> = {
  water: [
...
function metricValue(context: ContextKey, data: any[]) {
  ...
  if (context === "capacity")
    return {
      main: avg(data.map((d) => d.capacidadeTotal)),
  ...
  if (context === "pressure")
    return {
      main: avg(data.map((d) => d.descargaA)),
      label: "Descarga média A",
      unit: "psi",
      color: "text-rose-300",
    };
...
function comparisonData(context: ContextKey, group: GroupKey, chillers: ChillerData[]) {
  const source = group === "all" ? chillers : chillers.filter((c) => c.id === group);
  if (context === "water") {
    return source.map((c) => ({
      name: c.name.replace("Chiller ", ""),
      value: c.deltaT,
      fill: c.id === "red" ? "#fb2d5c" : c.id === "blue" ? "#38bdf8" : "#e5e7eb",
    }));
  }
  if (context === "capacity") {
    return source.map((c) => ({
      name: c.name.replace("Chiller ", ""),
      value: c.capacityTotal,
      fill: c.id === "red" ? "#fb2d5c" : c.id === "blue" ? "#38bdf8" : "#e5e7eb",
    }));
  }
  if (context === "pressure") {
    return source.map((c) => ({
      name: c.name.replace("Chiller ", ""),
      value: Math.round(avg(c.series.pressureHigh.map((p) => p.a * 50))),
      fill: c.id === "red" ? "#fb2d5c" : c.id === "blue" ? "#38bdf8" : "#e5e7eb",
    }));
  }
  return source.map((c) => ({
    name: c.name.replace("Chiller ", ""),
    value: c.hydraulic.bypassValve,
    fill: c.id === "red" ? "#fb2d5c" : c.id === "blue" ? "#38bdf8" : "#e5e7eb",
  }));
}
...
function TrendsPage() {
  ...
  const apiTrend = useMemo(
    () => normalizeTrendsPayload(apiPayload, context, group),
    [apiPayload, context, group],
  );
  const data = useMemo(
  ...
  return (
    <div className="space-y-4">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active &&
                  "border-violet-400/65 bg-violet-500/18 shadow-[0_0_28px_rgba(168,85,247,0.18)]",
              )}
            >
              <div
                className={cn(
                  "grid h-11 w-11 place-items-center rounded-xl bg-white/5",
              ...
              <div>
                <div className="font-display text-sm font-semibold group-hover:text-white">
                  {item.label}
                </div>
                <div className="mt-0.5 text-[11px] text-muted-foreground group-hover:text-white/80">
                  {item.subtitle}
                </div>
              </div>
            </button>
        ...
        <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="grid h-4 w-4 place-items-center rounded-full border border-violet-400/35 text-[10px] text-violet-300">
                i
              </span>
            </div>
          ...
          <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
            <span>Exibição</span>
            <span className="rounded-lg bg-violet-500/25 px-3 py-1.5 font-semibold text-violet-100">
              Linha
            </span>
            <span className="rounded-lg border border-border/60 px-3 py-1.5 hover:text-white">
              Área
            </span>
            <span className="ml-2">Intervalo</span>
            <span className="rounded-lg border border-border/60 px-3 py-1.5">{periodLabel}</span>
            <button className="grid h-8 w-8 place-items-center rounded-lg border border-border/60 text-muted-foreground hover:text-white">
            ...
            <LineChart data={data} margin={{ top: 10, right: 24, left: -8, bottom: 5 }}>
              ...
              <Tooltip
                ...
                formatter={(value: any, name: any) => [
                  `${fmt(Number(value), activeContext.unit === "psi" ? 0 : 1)} ${activeContext.unit}`,
                  name,
                ]}
              />
              {trendLines[context].map((line) => (
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        <MiniKpi
          label={metric.label}
          value={fmt(metric.main, context === "pressure" ? 0 : 1)}
          unit={metric.unit}
          delta="▼ 14% vs período anterior"
          color={metric.color}
          icon={TrendingDown}
        />
        <MiniKpi
      ...
      <div className="grid gap-4 xl:grid-cols-[1.05fr_1fr_1fr]">
        <div className="glass-card p-4">
          <h3 className="font-display text-sm font-bold uppercase tracking-[0.12em]">
            Insights Inteligentes do Período
          </h3>
          <div className="mt-4 space-y-2">
            {insights(context).map((text, index) => (
              <div
                key={text}
                className="flex items-center gap-3 rounded-lg border border-border/40 bg-background/30 px-3 py-3 text-sm text-muted-foreground"
              >
                <span
                  className={cn(
        ...
        <div className="glass-card p-4">
          ...
          <p className="text-[11px] text-muted-foreground">
            {contextConfig[context].label} · período selecionado
          </p>
          <div className="mt-5 h-[210px]">
            <ResponsiveContainer>
              <BarChart
                data={comparison}
                layout="vertical"
                margin={{ left: 18, right: 24, top: 6, bottom: 6 }}
              >
        ...
        <div className="glass-card p-4">
          <h3 className="font-display text-sm font-bold uppercase tracking-[0.12em]">
            {distributionTitle(context)}
          </h3>
          <p className="text-[11px] text-muted-foreground">
            Horas classificadas no período selecionado
          </p>
                ...
                return (
                  <div key={item.name} className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="font-mono font-semibold text-foreground">
                      {item.value}% <span className="text-muted-foreground">({itemHours} h)</span>
                    </span>
                  </div>
                );
              })}
              <p className="pt-2 text-xs text-muted-foreground">
                Quanto tempo o sistema operou em cada faixa operacional.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border/50 bg-background/30 px-4 py-3 text-xs text-muted-foreground">
        Os valores apresentados são calculados com base nos dados consolidados e respeitam o período
        global selecionado no topo da página.
      </div>
    </div>
  );
}
